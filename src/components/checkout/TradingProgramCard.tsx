    ...
    >
      <span
        className={cn(
          ...
          selected ? 'border-white bg-white' : 'border-muted-foreground/40 bg-transparent',
        )}
      >
        ...
        {program.stages.map((stage, i) => (
          <span key={stage} className="flex items-center gap-2">
            ...
            {i < program.stages.length - 1 && (
              <span className={selected ? 'text-blue-200' : 'text-muted-foreground'}>→</span>
            )}
          </span>
        ))}
      </div>
      ...
      {showRules && (
        <ul className={cn('mt-4 space-y-2 border-t pt-4', selected ? 'border-white/20' : 'border-border')}>
          {program.rules.map((r) => (
            <li
              key={r.label}
              className={cn(
